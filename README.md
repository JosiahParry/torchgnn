...
`{torchgnn}` provides `gcn_conv_model()` and `gcn_general_model()`
...
An S7 class `torchgnn::Aggregator` is created with a generic method with
signature `forward(x, adj, tensor)`. Only basic aggregators are
...
`{torchgnn}` works with sparse COO torch tensors for adjacency and
expects dense feature tensors. Note that weighted is handled by
incorporating it directly into the adjacency matrix.
...
The function `graph_split()` create split **masks**. This API will
likely change as how to integrate large graphs into `{torch}` and
`{luz}` is figured out.
